...
\textbf{Test Environment Specifications}: All indoor tests were conducted in controlled laboratory environment with 5-10m node spacing using 3-node linear, 4-node diamond, or 5-node mesh topologies (detailed in Section~\ref{sec:network-design}, Figure~\ref{fig:topology-layouts}). Outdoor tests used multi-node deployments at specified distances with building penetration.
...
\begin{table}[h]
\centering
\caption{Quantitative Protocol Comparison (20 Hardware Tests). Indoor tests: 3-5 node topologies (linear/diamond/mesh) at 5-10m spacing, 14 dBm, SF7. Outdoor tests: Protocol 3 only, 4 nodes at 105-935m, 14-20 dBm, SF7-9.}
...
\textbf{Test Configuration}: Outdoor deployments used 4-5 node topologies with 3-hop maximum path length. Topology: 1-2 sensor nodes, 1-2 relay nodes, 1 gateway node positioned at specified distances with building obstructions (AIT Campus test: 105-383m; Extreme-Range test: 935m).

\begin{figure}[H]
\centering
\includegraphics[width=0.9\textwidth]{figures/AIT_Map.jpg}
\caption{AIT Campus Physical Distance Multi-Hop Test Environment (105-383m)}
...
\caption{Outdoor Extreme-Range Multi-Hop Test Environment (Off-Campus Location, 935m)}
\label{fig:outdoor-map}
\end{figure}

Test Configuration (Test ID: gateways-cold\_20251119\_182553):
\begin{itemize}
\item Location: Off-campus residential building
