...
\item Manual node role configuration (compile-time only, no runtime adaptation)
\item MQTT bridge architecture (requires Raspberry Pi, no embedded MQTT in ESP32)
\item Hazemon platform incompatibility (LoRaWAN MQTT vs LoRa Mesh)
\end{enumerate}
...
\item \textbf{Scalability:} Two deployment scenarios projected: (1) Stable networks: 67-90\% overhead reduction (best case), (2) Mobile networks: 45-65\% overhead reduction (with 180s safety ceiling for fast fault detection). Both scenarios keep 10-50 node networks within 1\% duty cycle constraint
\end{itemize}
...
\begin{itemize}
\item 10-node deployment: Projected 45-67\% overhead reduction depending on safety HELLO configuration (stable networks: 67\%, mobile networks with 180s safety: 45\%)
\item PDR target: $>$95\% achievable at $<$500m sensor spacing (indoor tests: 96.7-100\% validated)
\item Fault tolerance: 378s detection with safety HELLO enables automated alerts to facilities management
\end{itemize}
...
\begin{tabular}{p{4cm}ccp{3.5cm}p{4cm}}
\hline
