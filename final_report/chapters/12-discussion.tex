...
\label{tab:protocol-comparison-discussion}
...
\subsection{Node Role Manual Configuration Requirement}

\textbf{Limitation}: Node roles (Sensor, Relay, Gateway) must be configured at compile-time and require firmware re-flashing to change.

\textbf{Implementation Details}:

The current implementation assigns node roles through compile-time macros in \texttt{config.h}:

\begin{itemize}
\item Role assignment based on hardcoded \texttt{NODE\_ID} values
\item Protocol 1 \& 2: Nodes 1-2 as Sensors, 3-4 as Relays, 5-6 as Gateways
\item Protocol 3: Similar fixed mapping using \texttt{ROLE\_GATEWAY} and \texttt{ROLE\_DEFAULT}
\item No runtime role reconfiguration capability
\end{itemize}

\textbf{Impact on Mobile Networks}:

For mobile network deployments where nodes might need to dynamically assume different roles (e.g., a sensor becoming a relay when positioned between other nodes), the current architecture requires:
\begin{enumerate}
\item Manual identification of required role change
\item Physical access to the node
\item Firmware re-compilation with updated \texttt{NODE\_ID} configuration
\item Re-flashing via USB connection
\end{enumerate}

This process is impractical for deployed networks and limits adaptability in dynamic scenarios.

\textbf{Gateway Discovery is Automatic}:

While role assignment is manual, gateway discovery operates automatically at runtime through the routing table. Nodes advertise their roles in HELLO packets, and the routing algorithm (\texttt{RoutingTableService::getBestNodeByRole()}) identifies available gateways without manual configuration. This provides partial flexibility---gateways are discovered automatically, but nodes cannot change their own roles dynamically.

\textbf{Recommended Mitigation}:

Implement runtime role configuration through:
\begin{itemize}
\item Configuration storage in ESP32 non-volatile memory (NVS)
\item Role assignment via UART command interface or Over-the-Air (OTA) updates
\item Role negotiation protocol where nodes can request role changes based on network conditions
\end{itemize}

This enhancement would enable truly adaptive mobile networks where nodes automatically assume optimal roles based on topology and connectivity.

\subsection{MQTT Integration Architecture and Hazemon Compatibility}

\textbf{Limitation}: No embedded MQTT client in ESP32 firmware; cloud connectivity requires external Raspberry Pi bridge.

\textbf{Current Architecture}:

The system implements a two-tier architecture for MQTT connectivity:

\begin{enumerate}
\item \textbf{LoRa Mesh Layer}: ESP32 nodes communicate via LoRa radio only (no MQTT)
\item \textbf{Gateway Bridge Layer}: Raspberry Pi connected to gateway node via USB serial
\item \textbf{MQTT Publishing Layer}: Python script (\texttt{mqtt\_publisher.py}) parses serial data and publishes to MQTT broker
\end{enumerate}

\textbf{Hazemon Platform Incompatibility}:

The AIT Hazemon sensor monitoring platform uses LoRaWAN architecture with specialized MQTT topics and message formats designed for LoRaWAN Class A/B/C devices. Key incompatibilities include:

\begin{itemize}
\item \textbf{Protocol Difference}: Hazemon expects LoRaWAN MQTT messages (with DevEUI, AppEUI, join procedures), whereas xMESH implements LoRa Mesh (peer-to-peer with MAC-based addressing)
\item \textbf{Message Format}: Hazemon MQTT topics follow LoRaWAN conventions (\texttt{application/[AppID]/device/[DevEUI]/rx}), incompatible with xMESH topics (\texttt{mesh/ingest/\{node\_id\}})
\item \textbf{Network Architecture}: LoRaWAN uses star topology (sensor → gateway → server), whereas LoRa Mesh uses multi-hop topology (sensor → relay → relay → gateway)
\end{itemize}
...
\begin{itemize}
\item Local Mosquitto MQTT broker (validated, Section~\ref{sec:mqtt-integration})
\item Generic MQTT brokers supporting standard publish/subscribe (AWS IoT Core, Azure IoT Hub, HiveMQ)
\item Custom monitoring dashboards via MQTT topic subscription
\end{itemize}

The Raspberry Pi bridge provides a generic MQTT interface, enabling integration with any standards-compliant MQTT infrastructure.

\textbf{Recommended Mitigation for Hazemon Integration}:

Two approaches for future Hazemon compatibility:

\begin{enumerate}
\item \textbf{Protocol Translation Layer}: Implement MQTT topic/payload translator on Raspberry Pi that converts xMESH mesh messages to Hazemon-compatible LoRaWAN MQTT format. This would enable Hazemon dashboard visualization without firmware changes.

\item \textbf{Dual-Mode Gateway}: Extend gateway firmware to support both LoRa Mesh (for xMESH sensors) and LoRaWAN (for Hazemon sensors) simultaneously, acting as a protocol bridge. This requires significant LoRaWAN stack integration (e.g., LMIC library).
\end{enumerate}

\textbf{Phase 2 Future Work}: Direct Hazemon integration was explicitly scoped as Phase 2 (Section~\ref{sec:scope}). The current Phase 1 implementation provides the necessary MQTT foundation, with protocol translation being the primary integration challenge.
