...
Control overhead scalability analysis combining measured hardware test data (3-5 nodes, solid markers) with mathematical model projections (10-50 nodes, dashed lines). Protocol 2 fixed-interval approach shows linear growth violating 1\% duty cycle constraint beyond $\sim$45 nodes. Protocol 3 Trickle adaptive scheduling maintains sublinear growth through neighbor-density-based suppression, projected to support 50+ node networks within regulatory limits. Blue shaded region: empirical validation from hardware tests. Green shaded region: RFC 6206-based model projection assuming stable networks. Projection methodology detailed in text---Trickle suppression probability increases with neighbor count, enabling scalability not achievable with fixed-interval approaches.

HELLO overhead projection from measured 3-5 node data (solid lines) to large-scale deployments (dashed lines, 10-50 nodes). Protocol 2 exhibits linear scaling (N$\times$30 HELLOs/hour), approaching AS923 duty cycle limit (640 HELLOs/hour, red line) at 21 nodes. Protocol 3 demonstrates sub-linear scaling through Trickle adaptive suppression, projecting 67\% overhead reduction at 10 nodes and remaining below duty cycle limit even at 50 nodes. \textbf{Key insight}: Measured 3-5 node trend (31-33\% reduction) extrapolates to 67-90\% reduction in larger networks if Trickle maintains I$_{\text{max}}$ stability, validating scalability hypothesis. \textbf{Implication}: Protocol 2's linear growth creates fundamental scalability ceiling at $\sim$20 nodes for 1 pkt/min traffic, while Protocol 3 enables city-scale deployments (50+ nodes) within regulatory constraints. Blue shaded region shows measured data; gray region shows mathematical projection requiring future validation.

\textbf{Projection Methodology}: Protocol 2 assumes fixed 120s interval (N$\times$15 HELLOs per 30min). Protocol 3 models Trickle efficiency improving with network density: 40\% (3 nodes) $\rightarrow$ 52\% (5 nodes) $\rightarrow$ 67\% (10 nodes) $\rightarrow$ 80\% (20 nodes) $\rightarrow$ 90\% (50 nodes), based on RFC 6206 suppression probability increasing with neighbor count.
