# Final Validation Tests (Post-Fix)

**Updated**: 2025-11-13  
**Branch**: `feature/w5-load-sharing`  
**Status**: Indoor suite complete (Tests 1-4) — only the outdoor/physical multi-hop test remains.

**Executed Indoor Runs (logs under `experiments/results/`):**
1. `protocol1/4node_linear_20251111_175245` – Flooding baseline, 30 min, 96.7 % PDR (Gateway log only).
2. `protocol2/4node_linear_20251111_192107` – Hop-count baseline, 30 min, PDR 81.7 % (worst-case) / 96.7 % (diamond), establishes 120 s HELLO baseline.
3. `protocol3/30min_overhead_validation_20251111_032647` – Single-gateway run showing 31 HELLOs vs 45 baseline (31 % reduction) with 100 % PDR.
4. `protocol3/w5_gateway_indoor_20251113_013301` + `protocol3/w5_gateway_indoor_over_I600s_node_detection_20251113_120301` – Two-gateway W5 validation and endurance test with node failures detected at 375–384 s while load keeps rebalancing (16 vs 13 packets).

**Remaining Test**: Outdoor/hallway multi-hop validation (Test 5). Needs ≤8 dBm TX power and 10–15 m spacing to force hops > 0.

## Test Matrix (Current Status)

| Test | Protocol | Nodes | Topology | Location | Duration | Monitor | Purpose | Status |
|------|----------|-------|----------|----------|----------|---------|---------|--------|
| 1 | P1 Flooding | 1,2,3,5 | S+S+R+G | Indoor | 30 min | Gateway | Flooding baseline | ✅ Completed |
| 2 | P2 Hop-Count | 1,2,3,5 | S+S+R+G | Indoor | 30 min | Gateway | Intelligent baseline | ✅ Completed |
| 3 | P3 Single-GW | 1,2,3,5 | S+S+R+G | Indoor | 30 min | Gateway | Overhead reduction | ✅ Completed |
| 4 | P3 Multi-GW | 1,2,3,4,5 | S+S+R+G+G | Indoor | 30 min | **Sensor** | **W5 validation** | ✅ Completed |
| 5 | P3 Multi-Hop | 1,2,3,5 | S+S+R+G | **Outdoor** | 30 min | Gateway | **Multi-hop mesh** | 🔄 Pending |
...
├── SCALABILITY_ASSESSMENT.md (code analysis, still valid)
├── results/
│   ├── protocol1/
