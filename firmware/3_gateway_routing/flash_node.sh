#!/bin/bash
# Flash script for Protocol 3 - Gateway-Aware Cost Routing on macOS
...
echo "======================================="
echo "Flashing Protocol 3 - Node $NODE_ID"
echo "Port: $PORT"
echo "======================================="

# CRITICAL: Remove entire build cache to prevent NODE_ID conflicts
echo "Removing build cache completely..."
rm -rf .pio/build

# Build with specific NODE_ID
echo "Building with NODE_ID=$NODE_ID..."
...
if [ $? -eq 0 ]; then
    ...
    echo "$(date): Flashed Protocol 3 to Node $NODE_ID on $PORT" >> ../../.context/session_log.md
    echo ""
    echo "To monitor output:"
    echo "pio device monitor --port $PORT --baud 115200"
else
    echo "❌ Failed to flash Node $NODE_ID"
    echo "$(date): FAILED to flash Protocol 3 to Node $NODE_ID" >> ../../.context/session_log.md
fi
