...
void DisplayManager::drawRoutingPage() {
    ...
    display->setCursor(0, 20);
    display->print(F("Gateway: 0x"));
    display->print(status->gatewayAddr, HEX);

    display->setCursor(0, 30);
    display->print(F("Next Hop: 0x"));
    display->print(status->nextHopAddr, HEX);
...
void DisplayManager::drawDebugPage() {
    ...
    display->setCursor(0, 30);
    display->print(F("Node ID: 0x"));
    display->print(status->nodeId, HEX);
...
void DisplayManager::drawHeader() {
    // Top line: Node ID and page indicator
    display->setCursor(0, 0);
    display->print(F("Node "));
    display->print(status->nodeId);
    display->print(F("  ["));
