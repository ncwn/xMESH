...
static const char *TAG = "SENSOR_MH-Z16";

#define MHZ16_START_BYTE    0xFF
#define MHZ16_READ_CMD      0x86
#define MHZ16_READ_CMD_POS  1
#define MHZ16_READ_HIGH_POS 2
#define MHZ16_READ_LOW_POS  3
#define MHZ16_MAX_LENGTH    9

static uint8_t READ_CO2_CMD[] = {0XFF, 0x01, MHZ16_READ_CMD, 0x00, 0x00, 0x00, 0x00, 0x00, 0x79};

#define CHECK_INITD(s_hdl) do {                       \
    if ((s_hdl)->status == SENSORDRIV_MHZ16_STAT_UNINITD) return CAN5_ERR_INVALID_STATE; \
} while(0)
...
typedef enum sensordriv_mhz16_status_e {
    SENSORDRIV_MHZ16_STAT_UNINITD = 0,
    SENSORDRIV_MHZ16_STAT_INITD,
    SENSORDRIV_MHZ16_STAT_STAT_READING,
} sensordriv_mhz16_status_t;
...
const can5_sensordriv_t sensordriv_mh_z16_co2 = {
    .ops = {
        .alloc = alloc,
...
typedef struct co2_rx_buf_s {
    volatile bool start_found;
    char buf[MHZ16_MAX_LENGTH + 1];
    size_t len;
} co2_rx_buf_t;

typedef struct sensor_hdl_s {
    uint8_t sensor_id;
    volatile sensordriv_mhz16_status_t status;
    volatile bool async_read;
    can5_port_idx_t port;
    can5_sensor_readcb_f *readcb;
    can5_sensor_mh_z16_co2_data_t cb_data;
    can5_sensor_hwcb_f *hwcb;
    co2_rx_buf_t rx_buf;
    time_t read_time_end;
    struct {
        can5_sensor_mh_z16_co2_data_t data;
        ssize_t n;
    } read;
} sensor_hdl_t;
...
static can5_err_t __sensor_read_rx(sensor_hdl_t *s_hdl, can5_sensor_mh_z16_co2_data_t *co2_data, time_t timeout);

static can5_err_t __read_co2(sensor_hdl_t *s_hdl, can5_sensor_mh_z16_co2_data_t *co2_data);
...
static can5_sensor_hdl_t *alloc(size_t *len)
{
    ...
    *len = sizeof(can5_sensor_hdl_t) + sizeof(sensor_hdl_t);
    ...
    s_hdl->status = SENSORDRIV_MHZ16_STAT_UNINITD;
...
static can5_err_t detect(can5_sensor_hdl_t *hdl, can5_port_idx_t port)
{
    ...
    can5_sensor_mh_z16_co2_data_t co2_data;
    s_hdl->port = port;

    CLEAR_STRUCT(co2_data);

    if (__read_co2(s_hdl, &co2_data) != CAN5_SUCCESS) {
        s_hdl->port = CAN5_PORT_NULL;
        return CAN5_SENSOR_ERR_INVALID_PORT;
    }


    ESP_LOGI(TAG, "Detected MH-Z16 CO2 module!");
...
static can5_err_t init(can5_sensor_hdl_t *hdl, can5_port_idx_t port, can5_sensor_hwcb_f *hwcb)
{
    ...
    s_hdl->hwcb = hwcb;
    s_hdl->status = SENSORDRIV_MHZ16_STAT_INITD;
...
static can5_err_t run(can5_sensor_hdl_t *hdl)
{
    ...
    if (s_hdl->async_read) {
        can5_sensor_mh_z16_co2_data_t co2_data_inst;
        can5_sensor_mh_z16_co2_data_t *co2_data;
        CLEAR_STRUCT(co2_data_inst);

        ret  = __read_co2(s_hdl, &co2_data_inst);

        if (ret == CAN5_SUCCESS && co2_data_inst.read) {
            s_hdl->read.n++;
            s_hdl->read.data.val += (co2_data_inst.val - s_hdl->read.data.val) / s_hdl->read.n;
        }
        ...
        if (s_hdl->read_time_end <= can5_time_ms(NULL)) {
            ...
            if (s_hdl->read.n > 0) {
                ...
                co2_data = &s_hdl->cb_data;
                *co2_data = s_hdl->read.data;
                ESP_LOGI(TAG, "Averaged %d datapoints", s_hdl->read.n);

                s_hdl->readcb(get_sensor_id(hdl), co2_data, sizeof(*co2_data));
...
static can5_err_t read(can5_sensor_hdl_t *hdl, void *prxdata, size_t *plen, time_t read_time_end, bool blocking)
{
    ...
    if (s_hdl->status == SENSORDRIV_MHZ16_STAT_STAT_READING) {
        return CAN5_SENSOR_ERR_BUSY;
    }

    s_hdl->status = SENSORDRIV_MHZ16_STAT_STAT_READING;
...
{
    sensor_hdl_t *s_hdl = hdl->hdl;
    const can5_sensor_mh_z16_co2_data_t *s_data = data;
    ...
    if (sizeof(can5_sensor_mh_z16_co2_data_t) != data_len) {
        goto done;
    }
    ...
    elem = can_5_sensor_data_create(CAN5_SENSOR_DATA_TYPE_MH_Z16_CO2, s_hdl->port,
...
static bool is_running(can5_sensor_hdl_t *hdl)
{
    ...
    return s_hdl->status >= SENSORDRIV_MHZ16_STAT_INITD;
}
...
static can5_err_t __parse_read_rx(sensor_hdl_t *s_hdl, char buf, char *out_buf)
{
    // TRACE_FUNC; /* Too frequent */
    bool read_err = false;

    co2_rx_buf_t *rx_buf;
    rx_buf = &s_hdl->rx_buf;

    if (rx_buf->len + 1 > MHZ16_MAX_LENGTH + 1) {
        return CAN5_SENSOR_ERR_PARSE_ERROR;
    }

    if (buf == MHZ16_START_BYTE) {
        /* Check start byte */
        rx_buf->start_found = true;
        rx_buf->len = 0;
    } else if (rx_buf->start_found && rx_buf->len == (MHZ16_MAX_LENGTH - 1)) {
    ...
    /* Check is read command is correct */
    if (rx_buf->len == MHZ16_READ_CMD_POS) {
        read_err = buf != MHZ16_READ_CMD;
    }

    /* Check if max length is not crossed */
    read_err |= rx_buf->len >= MHZ16_MAX_LENGTH;
...
static can5_err_t __sensor_read_rx(sensor_hdl_t *s_hdl, can5_sensor_mh_z16_co2_data_t *co2_data, time_t timeout)
{
    ...
    char buf;
    size_t len;
    char sentence[MHZ16_MAX_LENGTH];
    ...
    while (can5_time_ms(NULL)<= start + timeout) {
        ...
        if (len == 1) {
            if (__parse_read_rx(s_hdl, buf, sentence) == CAN5_SUCCESS) {
                co2_data->read = true;
                co2_data->val = sentence[MHZ16_READ_HIGH_POS] * 256 + sentence[MHZ16_READ_LOW_POS];
                ESP_LOG_BUFFER_HEXDUMP_V(TAG, sentence, MHZ16_MAX_LENGTH,  ESP_LOG_INFO);
                return CAN5_SUCCESS;
            }
        }
        taskYIELD();
...
static can5_err_t __read_co2(sensor_hdl_t *s_hdl, can5_sensor_mh_z16_co2_data_t *co2_data)
{
    ...
    VERIFY_SUCCESS(hal.serial_send(READ_CO2_CMD, sizeof(READ_CO2_CMD), s_hdl->port, SERIAL_TIMEOUT));

    VERIFY_SUCCESS(__sensor_read_rx(s_hdl, co2_data, SERIAL_TIMEOUT));
 ...
 *
 */
static const can5_tag_tab_t _SENSORDRIV_MHZ16_STAT_STAT_tags = {
    TAG_TAB_ITEM(SENSORDRIV_MHZ16_STAT_UNINITD),
    TAG_TAB_ITEM(SENSORDRIV_MHZ16_STAT_INITD),
    TAG_TAB_ITEM(SENSORDRIV_MHZ16_STAT_STAT_READING),
};
...
static const char *status_getstr(can5_sensor_hdl_t *hdl, int32_t status)
{
    ...
    return TAG_LOOKUP(s_hdl->status, _SENSORDRIV_MHZ16_STAT_STAT_tags);
}
...
void parse_co2_read_data(const can5_sensordriv_t *driv, const can5_sensor_mh_z16_co2_data_t *co2_data, void *pout, size_t *plen)
{
    ...
    snprintf(pout, CAN5_SENSOR_PARSED_DATA_MAX_LEN, "%s:%u",
             can5_hazemon_get_type(HAZEMON_MHZ16_CO2)->token, co2_data->val);
    *plen = strlen(pout);
}
